...
#if LCD_LANGUAGE == fr_na | LCD_LANGUAGE == fr //  French version for status messages
void DGUSRxHandler::ScreenChange(DGUS_VP &vp, void *data_ptr)
{
  ...
  {
    dgus_screen_handler.SetStatusMessagePGM(PSTR("Impossible pendant une impression"));
    return;
  }

  if (vp.addr == DGUS_Addr::SCREENCHANGE_Printing && (!printingIsActive() && !printingIsPaused()))
  {
    dgus_screen_handler.SetStatusMessagePGM(PSTR("Impossible tant que l'imprimante est en attente"));
    return;
  }
...
void DGUSRxHandler::PrintFile(DGUS_VP &vp, void *data_ptr)
{
  ...
  {
    dgus_screen_handler.SetStatusMessagePGM(PSTR("Aucun fichier selectionne"));
    return;
  }
...
void DGUSRxHandler::FilamentMove(DGUS_VP &vp, void *data_ptr)
{
  ...
  {
    dgus_screen_handler.SetStatusMessagePGM(PSTR("Temperature trop basse"));
    return;
  }
...
void DGUSRxHandler::GcodeExecute(DGUS_VP &vp, void *data_ptr)
{
  ...
  dgus_screen_handler.SetMessageLinePGM(NUL_STR, 1);
  dgus_screen_handler.SetMessageLinePGM(PSTR("Execution de la commande..."), 2);
  dgus_screen_handler.SetMessageLinePGM(NUL_STR, 3);
  dgus_screen_handler.SetMessageLinePGM(NUL_STR, 4);
...
void DGUSRxHandler::PIDRun(DGUS_VP &vp, void *data_ptr)
{
  ...
  case DGUS_Data::Heater::BED:
    ...
    break;
#else
    dgus_screen_handler.SetStatusMessagePGM(PSTR("PID du plateau desactive"));
    return;
#endif
  case DGUS_Data::Heater::H0:
#if ENABLED(PIDTEMP)
    ...
    break;
#else
    dgus_screen_handler.SetStatusMessagePGM(PSTR("PID de la buse desactive"));
    return;
#endif
#if HOTENDS > 1
  case DGUS_Data::Heater::H1:
