...
void DGUSScreenHandler::Loop() {
  ...
  if (current_screen == DGUS_Screen::LEVELING_PROBING
    ...
    SetStatusMessagePGM(ExtUI::getMeshValid() ?
                          PSTR("Nivellement realise avec succes")
                        : PSTR("Echec du nivellement..."));
...
void DGUSScreenHandler::SettingsReset() {
  ...
  SetStatusMessagePGM(PSTR("Reinitialisation de l'EEPROM"));
}
...
void DGUSScreenHandler::ConfigurationStoreWritten(bool success) {
  if (!success) {
    SetStatusMessagePGM(PSTR("Echec ecriture de l'EEPROM"));
  }
}

void DGUSScreenHandler::ConfigurationStoreRead(bool success) {
  if (!success) {
    SetStatusMessagePGM(PSTR("Echec lecture de l'EEPROM"));
  }
...
void DGUSScreenHandler::FilamentRunout(const ExtUI::extruder_t extruder) {
  char buffer[21];
  snprintf_P(buffer, sizeof(buffer), PSTR("Filament sur buse E%d"), extruder);
  ...
  void DGUSScreenHandler::PidTuning(const ExtUI::result_t rst) {
    switch (rst) {
      ...
      case ExtUI::PID_TEMP_TOO_HIGH:
        SetStatusMessagePGM(PSTR("Temperature trop haute"));
        break;
      case ExtUI::PID_TUNING_TIMEOUT:
        SetStatusMessagePGM(PSTR("Duree calibration PID hors limite..."));
        break;
    }
...
#endif // french language integration
